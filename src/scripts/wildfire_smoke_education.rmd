...
sim_dollar_plot_df <- sim_dollar_df %>% 
  select(fips, geoid, seda_year, moderator, as.character(1:NUM_SIMS)) %>% #sample(as.character(1:NUM_SIMS), 100)) %>% 
...
## sample one obs to actually plot out as rug plot
sim_plot_sample_df <- sim_dollar_plot_df[,.SD[sample(.N, min(1,.N))],by = c("fips","geoid","seda_year","moderator")]

## get the CI for cumulative dollar effect (for table next to figure)
sim_summary_df <- sim_dollar_df %>% group_by(moderator, seda_year) %>% 
